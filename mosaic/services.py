...
def Hough(param):
    lines = cv.HoughLinesP(param, 1, np.pi / 180., 120, minLineLength=50, maxLineGap=5)
...
def Haar(param):
    ...
    # 하르 공식
    haar = cv.CascadeClassifier(CTX + HAAR)  # 경로 + 가중치 파일명
    img = haar.detectMultiScale(param, minSize=(150, 150))
    if len(img) == 0:
        print("얼굴인식 실패")
        quit()
    for (x, y, w, h,) in img:
        print(f'얼굴의 좌표 : {x},{y},{w},{h}')
        cv.rectangle(param, (x, y), (x + w, y + h), (255, 0, 0), thickness=20) # RED(255, 0, 0) in RGB
    return (x, y, x + w, y + h)
...
def CannyOriginal(src, lowThreshold, highThreshold):
...
if __name__ == '__main__':
    ...
    img = CannyOriginal(GaussianBlur(img, 1, 1), 50, 150)
